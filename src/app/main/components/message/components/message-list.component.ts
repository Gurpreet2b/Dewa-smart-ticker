...
export class MessageListComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.authService.SetRestaurantName(`Message Templates`);
  ...
  GetMessageList(page: number) {
    ...
    this.http.get(`templates/?page=${page}&search=${search}`, null).subscribe((res: any) => {
  ...
  onDeleteMessageTemplate(id: number) {
    this.loading = true;
    this.http.delete(`templates/${id}/`).subscribe((res: any) => {
      if(res.status === true) {
